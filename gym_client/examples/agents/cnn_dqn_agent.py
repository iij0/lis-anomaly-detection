...
class CnnDqnAgent(object):
    ...
    def agent_init(self, **options):
        ...
        # print("q_net_input_dim :" , self.q_net_input_dim) 10240
    ...
    def agent_start(self, observation):
        ...
        Args:
            ...
            obserbation["image"] : camera_image -> numpy then shape (3, 227, 227) * 4 Flames
            obserbation["depth"] : list (1, 1024) it means (32, 32) depth image

        Returns:

        """
        #print(type(observation),np.shape(observation))
        #for key,elem in observation.items():
        #    try:
        #        print key , " : " , np.shape(elem) , " " , type(elem)
        #    except TypeError:
        #        #x_batch = np.ndarray((self.batchsize, 3, self.in_size, self.in_size), dtype=np.float32)
        #        image = np.asarray(observation[key][0]).transpose(2, 0, 1)[::-1].astype(np.float32)

        #        print key , " : " , image.shape , " " , image.dtype
